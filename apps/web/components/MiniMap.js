...
const MapComponent = dynamic(() => import('../components/Map'), { // RUTA CORREGIDA
  ssr: false,
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    fetchParkings();
  }, [userLocation, radioKm, filtroPmr, comunaFiltro]); 
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100%', overflow: 'hidden', fontFamily: 'sans-serif' }}>
                ...
                <div key={p.id} className="glass-panel" style={{ padding: '15px', border: '1px solid rgba(255,255,255,0.03)', cursor: 'pointer', transition: '0.2s', background: selectedSpot?.id === p.id ? 'rgba(59, 130, 246, 0.15)' : 'rgba(30, 41, 59, 0.5)', borderColor: selectedSpot?.id === p.id ? 'rgba(59, 130, 246, 0.5)' : 'rgba(255,255,255,0.03)' }} onClick={() => handleSpotClick(p)}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <h4 style={{ margin: '0 0 5px 0', color: 'white', fontSize: '1.05rem', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {p.nombre} {p.es_pmr && <i className="fa-solid fa-wheelchair" style={{ color: '#38bdf8', fontSize: '0.85rem' }}></i>}
                    </h4>
                  </div>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        {selectedSpot && (
          <div className="slide-in" style={{ position: 'absolute', bottom: '25px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(15, 23, 42, 0.95)', backdropFilter: 'blur(16px)', border: '1px solid rgba(59, 130, 246, 0.4)', borderRadius: '16px', padding: '25px', width: '90%', maxWidth: '450px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.7)', zIndex: 1000 }}>
            <button onClick={() => { setSelectedSpot(null); setShowAppSelector(false); }} style={{ position: 'absolute', top: '15px', right: '15px', background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '1.2rem' }}><i className="fa-solid fa-xmark"></i></button>

            <h3 style={{ margin: '0 0 5px 0', color: 'white', fontSize: '1.5rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '10px' }}>
              {selectedSpot.nombre} 
              {selectedSpot.es_pmr && <span style={{ background: 'rgba(56, 189, 248, 0.2)', color: '#38bdf8', fontSize: '0.75rem', padding: '4px 8px', borderRadius: '8px' }}><i className="fa-solid fa-wheelchair"></i> PMR</span>}
            </h3>
            ...
            {!showAppSelector ? (
              <div style={{ display: 'flex', gap: '12px' }}>
                <button onClick={() => setShowAppSelector(true)} className="btn-outline" style={{ flex: 1, padding: '14px', borderRadius: '10px', fontSize: '1rem', fontWeight: 'bold', borderColor: '#3b82f6', color: '#3b82f6', display: 'flex', justifyContent: 'center', gap: '8px', alignItems: 'center', background: 'rgba(59, 130, 246, 0.05)' }}>
                  <i className="fa-solid fa-location-arrow"></i> Navegar
                </button>
                <button onClick={handlePreReserve} disabled={isReserving || (selectedSpot.total_spots - selectedSpot.occupied_spots <= 0)} style={{ flex: 1.5, padding: '14px', background: (selectedSpot.total_spots - selectedSpot.occupied_spots <= 0) ? '#475569' : 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '1rem', fontWeight: 'bold', cursor: 'pointer', display: 'flex', justifyContent: 'center', gap: '8px', alignItems: 'center' }}>
                  {isReserving ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <><i className="fa-solid fa-bookmark"></i> Reservar Cupo</>}
                </button>
              </div>
            ) : (
              <div style={{ background: 'rgba(0,0,0,0.4)', borderRadius: '12px', padding: '15px', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <button onClick={openWaze} style={{ background: 'rgba(56, 189, 248, 0.15)', border: '1px solid #38bdf8', color: '#38bdf8', padding: '10px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 'bold', cursor: 'pointer' }}><i className="fa-brands fa-waze"></i> Abrir con Waze</button>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={openGoogleMaps} style={{ flex: 2, background: 'rgba(255, 255, 255, 0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'white', padding: '10px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', fontWeight: 'bold', cursor: 'pointer' }}><i className="fa-solid fa-map" style={{ color: '#fbbf24' }}></i> Google Maps</button>
                    <button onClick={copyAddress} disabled={isCopying} style={{ flex: 1, background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8', padding: '10px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', fontWeight: 'bold', cursor: 'pointer' }}>{isCopying ? <i className="fa-solid fa-spinner fa-spin"></i> : <><i className="fa-solid fa-copy"></i> Copiar</>}</button>
                  </div>
                </div>
              </div>
            )}
