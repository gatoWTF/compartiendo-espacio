...
export default function DashboardPage() {
  ...
  // Búsqueda Manual (Escribiendo la dirección)
  ...
  if (loading) return (
    <div style={{ height: '80vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
      <i className="fa-solid fa-circle-notch fa-spin" style={{ fontSize: '3rem', color: 'var(--primary)', marginBottom: '20px' }}></i>
      <h3 style={{ margin: 0, fontWeight: '500' }}>Inicializando tu base de mando...</h3>
    </div>
  );
      ...
      <div className="dashboard-header animated-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', borderBottom: '1px solid var(--border-glass)', paddingBottom: '20px' }}>
        <div>
          <h2 style={{ fontSize: '2.2rem', color: 'var(--primary)', margin: 0, fontWeight: '800' }}>
            <i className="fa-solid fa-layer-group" style={{ marginRight: '10px' }}></i> Panel de Control
          </h2>
          <p style={{ color: 'var(--text-muted)', margin: '5px 0 0 0', fontSize: '1.05rem' }}>Administración centralizada de tus espacios.</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
        ...
        <div className="glass-panel animated-card hover-elevate" style={{ flex: '1', minWidth: '380px', height: 'fit-content' }}>
          <h3 style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '15px', marginBottom: '20px' }}>
            <i className="fa-solid fa-location-dot" style={{ color: 'var(--primary)' }}></i> Nuevo Espacio
          </h3>
          
          <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            
            {/* INNOVACIÓN: Input con Botones Manual y GPS */}
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              {/* Botón de buscar manual */}
              <button type="button" onClick={handleSearchAddress} className="btn-primary" style={{ width: 'auto', padding: '0 15px', borderRadius: '10px' }} title="Buscar texto en Satélite">
                <i className="fa-solid fa-search"></i>
              </button>

              {/* Botón Mágico GPS (Innovación) */}
              <button type="button" onClick={() => {
                if (!navigator.geolocation) {
                  showToast("Tu navegador no soporta geolocalización.", "error");
                  return;
                }
                showToast("Detectando tu ubicación satelital...", "success");
                ...
                navigator.geolocation.getCurrentPosition(async (position) => {
                  ...
                  // Reverse Geocoding: Traducir coordenadas a dirección de texto
                  try {
                    ...
                    showToast("Ubicación fijada en el mapa (Sin texto).", "success");
                  }
                }, () => showToast("Permiso de ubicación denegado.", "error"));
                
              }} className="btn-outline hover-elevate" style={{ width: 'auto', padding: '0 15px', borderRadius: '10px', borderColor: '#10b981', color: '#10b981', background: 'rgba(16, 185, 129, 0.1)' }} title="Usar mi ubicación GPS actual">
            ...
            <div style={{ background: 'rgba(0,0,0,0.4)', padding: '10px', borderRadius: '12px', border: '1px solid rgba(59, 130, 246, 0.2)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', fontSize: '0.8rem', color: 'var(--primary)', fontWeight: '600' }}>
                <span>LAT: {lat ? Number(lat).toFixed(6) : 'Pendiente'}</span>
                <span>LNG: {lng ? Number(lng).toFixed(6) : 'Pendiente'}</span>
              </div>
            </div>
            ...
            <button type="submit" className="btn-primary hover-elevate" style={{ padding: '16px', fontSize: '1.1rem', borderRadius: '10px', fontWeight: 'bold', letterSpacing: '0.5px' }}>
        ...
        <div style={{ flex: '1.2', minWidth: '400px' }} className="animated-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', background: 'rgba(30, 41, 59, 0.4)', padding: '15px 20px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
            <h3 style={{ margin: 0 }}>
              <i className="fa-solid fa-list-check" style={{ color: 'var(--primary)' }}></i> Inventario
            </h3>
            
            {myParkings.length > 0 && (
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <button onClick={selectAll} className="btn-outline hover-elevate" style={{ padding: '6px 15px', fontSize: '0.85rem', width: 'auto', borderRadius: '8px' }}>
                  {selectedIds.length === myParkings.length ? 'Desmarcar Todos' : 'Seleccionar Todos'}
                </button>
                {selectedIds.length > 0 && (
                  <button onClick={handleBulkDelete} className="btn-primary hover-elevate" style={{ padding: '6px 15px', fontSize: '0.85rem', width: 'auto', background: '#dc2626', border: 'none', borderRadius: '8px' }}>
          ...
          {myParkings.length === 0 ? (
             <div className="glass-panel" style={{ textAlign: 'center', padding: '80px 20px', borderStyle: 'dashed', borderWidth: '2px', borderColor: 'rgba(255,255,255,0.1)' }}>
               <i className="fa-solid fa-ghost" style={{ fontSize: '5rem', color: 'rgba(255,255,255,0.05)', marginBottom: '20px' }}></i>
               <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem' }}>No tienes estacionamientos registrados en el sistema.</p>
               <p style={{ color: 'var(--primary)', fontSize: '0.9rem', marginTop: '10px' }}>Utiliza el panel izquierdo para crear tu primer espacio.</p>
             </div>
          ) : (
                ...
                return (
                  <div key={parking.id} className="glass-panel hover-elevate" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', border: isSelected ? '2px solid #ef4444' : '1px solid rgba(255,255,255,0.08)', background: isSelected ? 'rgba(239, 68, 68, 0.05)' : 'rgba(15, 23, 42, 0.6)', animationDelay: `${index * 0.1}s` }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
                      ...
                      <div>
                        <h4 style={{ color: 'var(--text-main)', margin: 0, fontSize: '1.2rem', fontWeight: '700' }}>{parking.nombre}</h4>
                        <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
                          <span className={`badge ${isFull ? '' : 'green'}`} style={{ backgroundColor: isFull ? 'rgba(239, 68, 68, 0.2)' : '', color: isFull ? '#f87171' : '', padding: '4px 10px' }}>
                            {isFull ? <i className="fa-solid fa-ban"></i> : <i className="fa-solid fa-check"></i>} {isFull ? 'Lleno' : 'Disponible'}
                          </span>
                          <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', background: 'rgba(0,0,0,0.3)', padding: '2px 8px', borderRadius: '12px' }}>
                            <i className="fa-solid fa-car" style={{ marginRight: '5px' }}></i> {parking.total_spots} cupos
                          </span>
                        </div>
                      </div>
                    </div>

                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(0,0,0,0.4)', padding: '10px 18px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                      <button onClick={() => updateOccupancy(parking.id, parking.occupied_spots, parking.total_spots, -1)} disabled={parking.occupied_spots === 0} style={{ background: 'transparent', border: 'none', color: parking.occupied_spots === 0 ? 'rgba(255,255,255,0.1)' : '#10b981', fontSize: '1.6rem', cursor: parking.occupied_spots === 0 ? 'not-allowed' : 'pointer', transition: '0.2s' }}>
                        <i className="fa-solid fa-square-minus hover-elevate"></i>
                      </button>
                      ...
                      <button onClick={() => updateOccupancy(parking.id, parking.occupied_spots, parking.total_spots, 1)} disabled={isFull} style={{ background: 'transparent', border: 'none', color: isFull ? 'rgba(255,255,255,0.1)' : '#ef4444', fontSize: '1.6rem', cursor: isFull ? 'not-allowed' : 'pointer', transition: '0.2s' }}>
                        <i className="fa-solid fa-square-plus hover-elevate"></i>
                      </button>
                    </div>
                  </div>
                );
