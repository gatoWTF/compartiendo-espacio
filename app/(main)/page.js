...
export default function HomePage() {
  ...
  const [isCopying, setIsCopying] = useState(false); // Estado para la animación de copiado
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      // Si corriste el código SQL, esto ahora traerá datos para los Clientes también
      const { data: parkings } = await supabase.from('estacionamientos').select('*');
      if (parkings) { setAllParkings(parkings); setFilteredParkings(parkings); }
    };
    initData();
  }, []);

  const calcularDistanciaLineaRecta = (lat1, lon1, lat2, lon2) => {
    const R = 6371;
  ...
  useEffect(() => {
    ...
    if (comunaFiltro.trim() !== '') { result = result.filter(p => p.nombre.toLowerCase().includes(comunaFiltro.toLowerCase())); }
    if (userLocation && radioKm < 50) { 
      result = result.filter(p => {
        const dist = calcularDistanciaLineaRecta(userLocation.lat, userLocation.lng, p.lat, p.lng);
        p.distancia = dist.toFixed(1);
        return dist <= radioKm;
      });
      result.sort((a, b) => a.distancia - b.distancia);
    }
    setFilteredParkings(result);
  }, [comunaFiltro, radioKm, allParkings, userLocation]);
  ...
  // INNOVACIÓN: Obtener Dirección Real en vez de Coordenadas
  const copyAddress = async () => {
    setIsCopying(true);
    showToast("Traduciendo coordenadas a dirección...", "success");
    try {
      // Reverse Geocoding a OpenStreetMap
      ...
      // PLAN B: Copiar coordenadas si falla el satélite
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100%', overflow: 'hidden', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ width: '380px', background: '#0f172a', borderRight: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', padding: '20px', zIndex: 10 }}>
        ...
        {/* Lista de Destacados */}
        <h3 style={{ color: '#e2e8f0', fontSize: '1.1rem', marginBottom: '15px' }}><i className="fa-solid fa-star" style={{ color: '#fbbf24' }}></i> Lugares Cercanos</h3>
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {filteredParkings.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#64748b', marginTop: '30px', fontSize: '0.9rem' }}>
              <i className="fa-solid fa-car-burst" style={{ fontSize: '2rem', marginBottom: '10px', opacity: 0.3 }}></i>
              <p>No encontramos estacionamientos<br/>en este radio de zona.</p>
            </div>
          ) : (
              ...
              return (
                <div key={p.id} className="glass-panel" style={{ padding: '15px', border: '1px solid rgba(255,255,255,0.03)', cursor: 'pointer', transition: '0.2s', background: selectedSpot?.id === p.id ? 'rgba(59, 130, 246, 0.15)' : 'rgba(30, 41, 59, 0.5)', borderColor: selectedSpot?.id === p.id ? 'rgba(59, 130, 246, 0.5)' : 'rgba(255,255,255,0.03)' }} onClick={() => handleSpotClick(p)}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <h4 style={{ margin: '0 0 5px 0', color: 'white', fontSize: '1.05rem', fontWeight: '700' }}>{p.nombre}</h4>
        ...
        {selectedSpot && (
          <div className="slide-in" style={{ position: 'absolute', bottom: '25px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(15, 23, 42, 0.95)', backdropFilter: 'blur(16px)', border: '1px solid rgba(59, 130, 246, 0.4)', borderRadius: '16px', padding: '25px', width: '90%', maxWidth: '450px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.7)', zIndex: 1000, animation: 'slideUp 0.3s ease-out' }}>
            ...
            <style dangerouslySetInnerHTML={{__html: ` 
              ...
              @keyframes expandMenu { from { height: 0; opacity: 0; } to { height: 140px; opacity: 1; } }
            `}} />

            <button onClick={() => { setSelectedSpot(null); setShowAppSelector(false); }} style={{ position: 'absolute', top: '15px', right: '15px', background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '1.2rem', transition: '0.2s' }}>
              <i className="fa-solid fa-xmark hover-elevate"></i>
            </button>

            <h3 style={{ margin: '0 0 5px 0', color: 'white', fontSize: '1.5rem', fontWeight: '800' }}>{selectedSpot.nombre}</h3>
            ...
            {!showAppSelector ? (
              <div style={{ display: 'flex', gap: '12px', animation: 'slideUp 0.2s' }}>
                <button 
                  ...
                  style={{ flex: 1, padding: '14px', borderRadius: '10px', fontSize: '1rem', fontWeight: 'bold', borderColor: '#3b82f6', color: '#3b82f6', display: 'flex', justifyContent: 'center', gap: '8px', alignItems: 'center', background: 'rgba(59, 130, 246, 0.05)', transition: '0.2s' }}
                >
                ...
                <button 
                  ...
                  disabled={isReserving || (selectedSpot.total_spots - selectedSpot.occupied_spots <= 0)}
                  style={{ flex: 1.5, padding: '14px', background: (selectedSpot.total_spots - selectedSpot.occupied_spots <= 0) ? '#475569' : 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '1rem', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s', boxShadow: '0 8px 15px rgba(16, 185, 129, 0.3)', display: 'flex', justifyContent: 'center', gap: '8px', alignItems: 'center' }}
                >
              ...
              <div style={{ background: 'rgba(0,0,0,0.4)', borderRadius: '12px', padding: '15px', border: '1px solid rgba(255,255,255,0.05)', animation: 'expandMenu 0.3s ease-out forwards', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <button onClick={openWaze} style={{ background: 'rgba(56, 189, 248, 0.15)', border: '1px solid #38bdf8', color: '#38bdf8', padding: '10px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s' }}>
                    <i className="fa-brands fa-waze" style={{ fontSize: '1.2rem' }}></i> Abrir con Waze
                  </button>
                  
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={openGoogleMaps} style={{ flex: 2, background: 'rgba(255, 255, 255, 0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'white', padding: '10px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s' }}>
                      <i className="fa-solid fa-map" style={{ color: '#fbbf24' }}></i> Google Maps
                    </button>
                    
                    <button onClick={copyAddress} disabled={isCopying} title="Copiar Dirección Real" style={{ flex: 1, background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8', padding: '10px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s' }}>
                      {isCopying ? <i className="fa-solid fa-spinner fa-spin"></i> : <><i className="fa-solid fa-copy"></i> Copiar</>}
                    </button>
                  </div>
                </div>
              </div>
